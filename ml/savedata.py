...
def get_filename(id):
        ...
        if current_time.hour > 6 and current_time.hour < 18:
            ...
            time_of_day = 'Night'
        return f"ml/trained_data/data-{id}-{time_of_day}"
...
def savefile(filepath, data, device_id):
    if exists(filepath):
        if create_new_file(f"ml/trained_data/{getLastCSVFile()}"):
            ...
            if filepath != f"ml/trained_data/{getLastCSVFile()}":
                filepath = f"ml/trained_data/{getLastCSVFile()}"  
